...
// Conexão com MongoDB
mongoose.connect(process.env.MONGO_URL).then(() => console.log("Banco OK")).catch(err => console.error("Erro Banco"));
...
<head>
    ...
    <style>
        ...
        .wheel-canvas { width: 250px; height: 250px; border-radius: 50%; border: 8px solid #ffb800; transition: transform 5s cubic-bezier(0.1, 0, 0.1, 1); background: conic-gradient(
            #222 0deg 60deg, #333 60deg 120deg, #222 120deg 180deg, #333 180deg 240deg, #222 240deg 300deg, #333 300deg 360deg
...
<body>
    ...
    <script type="text/babel">
        ...
        function App() {
            ...
            // Dados Saque
            const [tipoChave, setTipoChave] = useState("document");
            ...
            const realizarSaque = async () => {
                if (valorSaque < 30) return alert("Mínimo R$ 30,00 para saque.");
                if (valorSaque > balance) return alert("Saldo insuficiente.");
                if (!chavePix) return alert("Digite sua chave Pix.");
                
                const res = await fetch('/sacar', { 
                    method: 'POST', 
                    headers: {'Content-Type': 'application/json'}, 
                    body: JSON.stringify({ valor: valorSaque, chavePix, tipoChave, username: user.username }) 
                });
                const data = await res.json();
                if(data.success) {
            ...
            if (!user) return (
                <div className="h-screen flex items-center justify-center p-4 bg-[#050505]">
                    <div className="bg-[#111] p-10 rounded-[40px] text-center w-full max-w-sm border border-white/5">
            ...
            return (
                <div className="min-h-screen bg-[#000] pb-20">
                    <header className="gold-header p-4 space-y-4 sticky top-0 z-50 shadow-xl">
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="leading-tight">
                                    <p className="font-black text-sm">{user.username} <i className="fa fa-caret-down"></i></p>
                                    <p className="text-[9px] font-bold opacity-60 uppercase">ID: {user.player_id}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                        ...
                        <div className="flex justify-around items-center">
                            <div className="text-center space-y-1" onClick={()=>setShowSacar(true)}>
                            ...
                            <div className="text-center space-y-1" onClick={()=>setShowWheel(true)}>
                                <div className="quick-btn relative border-red-500"><i className="fa fa-gift text-red-600 text-xl"></i><span className="absolute -top-1 -right-2 bg-green-500 text-white text-[7px] px-1 rounded-sm">GIRO</span></div>
                                <p className="text-[9px] font-black uppercase text-red-600">Oferta</p>
                            </div>
                            <div className="text-center space-y-1" onClick={()=>alert("Disponível no VIP 2")}>
                                <div className="quick-btn font-black text-xl italic text-black">15</div>
                            ...
                            <div className="max-w-md mx-auto space-y-4 animate-in fade-in">
                                <div className="vip-card">
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-full border border-green-500 flex items-center justify-center text-green-500 font-black italic">0</div>
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-[9px] text-gray-500 uppercase font-bold">VIP requer <span className="underline">Apostas</span> 81,35</p>
                                        </div>
                                    </div>
                                ...
                                <div className="rounded-[20px] overflow-hidden shadow-2xl">
                                    ...
                                    <div className="menu-row"><span><i className="fa fa-users mr-3"></i> Central de Convite</span> <i className="fa fa-chevron-right"></i></div>
                                </div>
                                <button onClick={() => setView('home')} className="w-full py-4 text-gray-500 font-bold uppercase text-xs mt-4 underline">Voltar</button>
                                <button onClick={() => window.location.reload()} className="w-full text-red-500 font-bold uppercase text-[10px] mt-4 italic">Sair da Conta</button>
                            </div>
                        ) : (
                    ...
                    {/* MODAL SAQUE COMPLETO */}
                    {showSacar && (
                        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-6 z-[110] backdrop-blur-md">
                            <div className="bg-[#111] p-8 rounded-[40px] w-full max-w-sm border border-white/10 relative">
                                <button onClick={() => setShowSacar(false)} className="absolute top-4 right-4 text-gray-500">✕</button>
                                <h2 className="text-xl font-black text-[#ffb800] mb-6 uppercase italic">Solicitar Saque</h2>
                                <div className="space-y-4">
                                    <div className="text-left">
                                        <label className="text-[10px] text-gray-500 uppercase font-bold ml-2 italic">Tipo de Chave</label>
                                        <select value={tipoChave} onChange={e=>setTipoChave(e.target.value)} className="w-full p-4 bg-black border border-white/10 rounded-2xl text-white outline-none">
                                    ...
                                    <div className="text-left">
                                        <label className="text-[10px] text-gray-500 uppercase font-bold ml-2 italic">Sua Chave Pix</label>
                                        <input type="text" value={chavePix} onChange={e=>setChavePix(e.target.value)} placeholder="Digite sua chave..." className="w-full p-4 bg-black border border-white/10 rounded-2xl text-white outline-none" />
                                    </div>
                                    <div className="text-left">
                                        <label className="text-[10px] text-gray-500 uppercase font-bold ml-2 italic">Valor do Saque (Saldo: R$ \${balance.toFixed(2)})</label>
                                        <input type="number" value={valorSaque} onChange={e=>setValorSaque(e.target.value)} placeholder="0,00" className="w-full p-4 bg-black border border-white/10 rounded-2xl text-white outline-none text-2xl font-bold" />
                                    </div>
                                    <button onClick={realizarSaque} className="w-full bg-[#ffb800] text-black font-black py-4 rounded-2xl uppercase mt-4 italic shadow-lg">Confirmar Saque</button>
                                    <p className="text-[9px] text-gray-500 uppercase mt-2 italic">Processamento em até 24 horas.</p>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-6 z-[101] backdrop-blur-md">
                            <div className="bg-[#111] p-8 rounded-[40px] text-center relative border border-white/10 w-full max-w-sm">
                                ...
                                <div className="wheel-container">
                                    ...
                                    <div className="wheel-canvas" style={{transform: \`rotate(\${rotation}deg)\`}}>
                                        <div className="wheel-label" style={{transform: 'rotate(30deg) translateY(-80px)'}}>R$ 50</div>
                                        <div className="wheel-label" style={{transform: 'rotate(90deg) translateY(-80px)'}}>R$ 5</div>
                                        <div className="wheel-label" style={{transform: 'rotate(150deg) translateY(-80px)'}}>R$ 20</div>
                                        <div className="wheel-label" style={{transform: 'rotate(210deg) translateY(-80px)'}}>R$ 10</div>
                                        <div className="wheel-label" style={{transform: 'rotate(270deg) translateY(-80px)'}}>R$ 40</div>
                                        <div className="wheel-label" style={{transform: 'rotate(330deg) translateY(-80px)'}}>R$ 30</div>
                                    </div>
                                </div>
                                <button onClick={girarRoleta} disabled={isSpinning} className="mt-8 gold-bg w-full py-5 rounded-3xl uppercase font-black italic shadow-xl">GIRAR AGORA</button>
                            </div>
                        </div>
                    )}
                    ...
                    {showDep && (
                        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-6 z-[100] backdrop-blur-sm">
                            <div className="bg-[#111] p-10 rounded-[48px] w-full max-w-sm text-center relative border border-white/5 shadow-2xl">
                                    ...
                                    <div className="space-y-6 flex flex-col items-center">
                                        {pix === "LOADING" ? <p className="animate-pulse gold-text font-black">Gerando...</p> : (
                                            <>
                                                ...
                                                <p className="gold-text font-black text-xs animate-pulse uppercase tracking-[3px] mt-4">Aguardando Pagamento...</p>
                                            </>
                                        )}
                                    </div>
                                )}
...
app.post('/sacar', async (req, res) => {
    try {
        const { valor, chavePix, tipoChave, username } = req.body;
        const user = await User.findOne({ username });
        if (!user || user.balance < valor) return res.status(400).json({ error: "Saldo insuficiente" });

        // Envia pagamento via SuitPay
        const response = await axios.post('https://api.suitpay.app/api/v1/gateway/pix-payment', {
            value: parseFloat(valor),
            key: chavePix,
            typeKey: tipoChave // SuitPay usa: document, phoneNumber, email, randomKey
        ...
        res.status(500).json({ error: "Falha no banco ao processar saque" });
    }
});

app.post('/add-prize', async (req, res) => {
...
app.post('/pix', async (req, res) => {
    try {
        const response = await axios.post('https://api.suitpay.app/api/v1/gateway/request-qrcode', {
            ...
            client: { name: req.body.nome, document: "15254131770", email: "cliente@email.com" }
        }, { headers: { 'Authorization': 'Bearer ' + TOKEN } });
